...
fail() { echo "FAIL: owner-state-v6: $*" >&2; exit 1; }
chmod() { :; }
sync() { :; }
...
FIREWALL_TAG=AbCdEf1234; ZAPRET2_OUT=Z2O_AbCdEf1234; ZAPRET2_IN=Z2I_AbCdEf1234; PENDING_OWNER_GENERATION=owner-v6
...
write_owner_state 123 456 00 200 owner-v6 active || fail "could not write v6 owner"
chmod 0600 "$OWNER_STATE"
cp "$OWNER_STATE" "$CASE/owner.v6"
...
[ "$(sed -n '1p' "$CASE/owner.v6")" = version=6 ] || fail "writer did not publish v6"
...
grep -Fqx "boot_id=$BOOT_A" "$CASE/owner.v6" || fail "writer did not bind current boot"
...
read_owner_state && owner_state_is_current_boot || fail "v6 write/read round trip failed"
owner_loaded_generation_for_write || fail "current owner could not be loaded for rewrite"
write_owner_state 123 456 00 200 owner-v6 active || fail "canonical owner rewrite failed"
cmp -s "$CASE/owner.v6" "$OWNER_STATE" || fail "v6 byte-for-byte round trip changed"
...
sed '/^firewall_tag=/d' "$CASE/owner.v6" > "$CASE/missing"
sed 's/^firewall_tag=.*/firewall_tag=unsafe!/' "$CASE/owner.v6" > "$CASE/malformed"
sed 's/^out_chain=.*/out_chain=Z2O_wrongchain/' "$CASE/owner.v6" > "$CASE/bad-out-chain"
sed 's/^in_chain=.*/in_chain=Z2I_wrongchain/' "$CASE/owner.v6" > "$CASE/bad-in-chain"
sed '/^firewall_tag=/p' "$CASE/owner.v6" > "$CASE/duplicate"
cp "$CASE/owner.v6" "$CASE/unknown"; printf 'future=value\n' >> "$CASE/unknown"
for candidate in "$CASE/missing" "$CASE/malformed" "$CASE/bad-out-chain" "$CASE/bad-in-chain" "$CASE/duplicate" "$CASE/unknown"; do
...
reset_case "$CASE/owner.v6"; Z2_TEST_PROCESS=live
recover_stale_owner_publication || fail "same-boot live v6 was not recognized"
[ -f "$OWNER_STATE" ] || fail "same-boot live owner was retired"

# A modules_update installer loads helpers from a candidate path while owner v6
...
reset_case "$CASE/owner.v6"
assert_preserved_failure "same-boot dead v6"
...
echo "Owner state v6/legacy-v5 shell tests passed"
