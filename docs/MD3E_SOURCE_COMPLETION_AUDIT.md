...
| App module package update | `UpdateManager`, `UpdateExecution`, `ModuleMutationCoordinator` | Magisk `--install-module`, `customize.sh`, runtime manifest | Repository-bound release URLs, exact SHA-256, package, signer/lineage, ABI, version, and versionCode checks complete before publication. The archive receives one structural preflight and one final SHA-256 identity check; Magisk alone stages the exact generation in `/data/adb/modules_update/zapret2`. The active tree and running service are never stopped, copied, replaced, rolled back, or locked by package staging. Exact pending generation metadata is verified after the cancellation-safe commit, and every successful module update requires reboot |
| Module activation | Magisk boot lifecycle | `/data/adb/modules_update/zapret2` | There is no app-owned hot-swap, backup generation, update transaction, cleanup journal, or migration path. Magisk selects the staged generation at boot; status and ordinary lifecycle observation remain independent of package staging |
