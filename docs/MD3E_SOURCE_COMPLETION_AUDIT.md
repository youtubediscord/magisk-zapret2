...
| Install/update preservation | `ModulePackageContract`, `ModuleUpdatePreservation` | `customize.sh`, runtime manifest, `package-contract.sh` | Preserve only a valid new-contract runtime, hostlists, safe custom-name presets, and the disable marker. Packaged presets, strategy catalogs, binaries, and all Lua files always come from the new release. Every preserved custom preset is revalidated; the active preset must pass the real installed `nfqws2 --dry-run` before publication |
| Preset/profile execution | `ProfileRepository`, `ControlDiagnosticsRepository` | `command-builder.sh`, build/hot-update precommit | The active TXT is the sole filter and strategy source. Unknown/Windows/ipcache options and unsafe dependencies fail closed; enabled-profile filters produce the exact firewall port union; a private line-based argv artifact is bound to the TXT and runtime hashes and executed without shell evaluation or `@config` |
