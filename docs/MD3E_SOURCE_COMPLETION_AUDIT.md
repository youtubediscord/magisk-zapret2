...
- Source-level target: Magisk 20.4+, KernelSU, and APatch.
- Signed release/device evidence remains required before a concrete manager and
  device combination is added to the verified release matrix.
...
| Irreversible clean purge | `ModulePurgeController`, `ControlViewModel` | `scripts/lifecycle/purge-contract.sh`, `scripts/lifecycle/zapret-purge.sh`, module `action.sh`, `uninstall.sh` | One canonical two-phase/one-time protocol shared by APK and root-manager Action; verified process/firewall teardown precedes allowlisted removal of module, state, install/update/recovery and legacy artifacts; APK bytes are preserved while app-owned data is reset; reboot is mandatory |
| App module package update | `UpdateManager`, `UpdateExecution`, `ModuleMutationCoordinator` | `RootModuleContract`, `customize.sh`, runtime manifest | Repository-bound release URLs, exact SHA-256, package, signer/lineage, ABI, version, and versionCode checks complete before publication. The archive receives one structural preflight and one final SHA-256 identity check; the detected Magisk, KernelSU, or APatch backend alone stages the exact generation in `/data/adb/modules_update/zapret2`. The active tree and running service are never stopped, copied, replaced, rolled back, or locked by package staging. Exact pending generation metadata is verified after the cancellation-safe commit, and every successful module update requires reboot |
| Module activation | Root-manager boot lifecycle | `/data/adb/modules_update/zapret2` | There is no app-owned hot-swap, backup generation, update transaction, cleanup journal, or migration path. The active root manager selects the staged generation at boot; status and ordinary lifecycle observation remain independent of package staging |
| Package contents | `ModulePackageContract` | `customize.sh`, runtime manifest, `package-contract.sh` | The release package is authoritative. Packaged presets, catalogs, binaries, scripts, wrappers, configuration seeds, and Lua files come from the selected release; the active package tree is never merged with or restored from an obsolete generation |
| Preset/profile execution | `ProfileRepository`, `ControlDiagnosticsRepository` | `command-builder.sh`, release precommit | The active TXT is the sole filter and strategy source. Unknown/Windows/ipcache options and unsafe dependencies fail closed; enabled-profile filters produce the exact firewall port union; a private line-based argv artifact is bound to the TXT and runtime hashes and executed without shell evaluation or `@config` |
...
`OwnerStateSchemaTest`, `LifecycleMutationLockProtocolTest`,
`CancellationSafeTerminalCommitTest`, `RuntimeCostBoundaryTest`, `ReleaseCiPolicyTest`, the
