...
class ControlFragment : Fragment() {
    ...
    /**
     * Updates the process statistics UI
     */
    private fun updateProcessStats(stats: ProcessStats) {
        if (stats.running) {
            // Show process stats section
            textProcessStatsHeader.visibility = View.VISIBLE
            layoutProcessStats.visibility = View.VISIBLE
            ...
            // Update uptime
            textProcessUptime.text = stats.uptimeFormatted

            // Update thread count
            textThreadCount.text = stats.threads.toString()

            // Update PID
            textProcessPid.text = stats.pid ?: "--"
        } else {
            // Hide process stats section when not running
            textProcessStatsHeader.visibility = View.GONE
            layoutProcessStats.visibility = View.GONE
        }
    }
    ...
    /**
     * Formats uptime in milliseconds to HH:MM:SS format
     */
    private fun formatUptimeHMS(millis: Long): String {
        if (millis < 0) return "--:--:--"
