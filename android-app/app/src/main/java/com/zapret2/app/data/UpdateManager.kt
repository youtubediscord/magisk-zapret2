    ...
    private suspend fun installModuleLocked(
        ...
        try {
            ...
            val prepareCommand = buildString {
                ...
                append("    if [ \"${'$'}status\" -eq 0 ]; then category_validation=${'$'}(/system/bin/sh ")
                    .append(RootFileIo.shellQuote(commandBuilder)).append(" --validate-categories-machine ")
                    .append(RootFileIo.shellQuote("$updateDir/zapret2")).append(") || status=${'$'}?; fi\n")
                append("    if [ \"${'$'}status\" -eq 0 ]; then [ \"${'$'}category_validation\" = \"${'$'}(printf 'Z2_CATEGORIES\\tOK')\" ] || status=1; fi\n")
                append("  fi\n")
                append("fi\n")
                append("if [ \"${'$'}status\" -eq 0 ]; then\n")
