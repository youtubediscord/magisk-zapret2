...
def filter_out_range(options):
    """Remove --out-range lines from strategy options (will be added dynamically from PKT_COUNT)"""
    lines = options.split('\n')
    filtered = [line for line in lines if not line.strip().startswith('--out-range')]
    return '\n'.join(filtered)
...
def generate_strategies_sh(tcp_strats, udp_strats):
    ...
    for sid, sdata in tcp_strats.items():
        # Filter out --out-range (added dynamically from PKT_COUNT)
        opts = filter_out_range(sdata['options']).replace('\n', ' \\\n')
        lines.append(f'        {sid})')
